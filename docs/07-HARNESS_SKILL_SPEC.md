...
| `harness-slim-gardenring` | Clean a bounded set of proven duplicates, dead artifacts, guessed shapes, or stale material | Broad refactors, speculative abstractions, or unproven deletion |
...
read-only. Gardenring updates feature and progress state only when the existing
harness requires it.
